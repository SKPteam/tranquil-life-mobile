// ignore_for_file: file_names, use_key_in_widget_constructors, avoid_print, unrelated_type_equality_checks
...
import '../schedule_time_with_consultant_screen.dart';
import '../set_time_slots_for_consultants_screen.dart';
import 'demoTimings.dart';
...
class _ScheduleMeetingDialogState extends State<ScheduleMeetingDialog> {
  ...
  Future getDataFromFirebase() async {
    print('consultant ID -- > ${widget.consultantProfileModel}');
    var allTimeSlotsRefFromDB =
        await allTimeSlotsRef.doc(widget.consultantProfileModel.uid).get();
    if (allTimeSlotsRefFromDB.exists) {
      var data = allTimeSlotsRefFromDB.data() as Map;
      ...
      for (String key in keys) {
        ...
        if (hours[key]) {
          print(hours[key]);
          allTimeSlotsList.add(TimeManipulativeHelperController.instance
              .convertFromUTCHourToHourpart2(key));
        }
      }
      List<String> remainingTimeSlot = [];
      allTimeSlotsList.sort();
      for (var i = 0; i < allTimeSlotsList.length; i++) {
        if (allTimeSlotsList[i].hour > 4 && allTimeSlotsList[i].hour < 18) {
          availableDayTimeList.add(TimeManipulativeHelperController.instance
              .convertFromTimeToHour(allTimeSlotsList[i]));
        } else if (allTimeSlotsList[i].hour >= 18) {
          availableNightTimeList.add(TimeManipulativeHelperController.instance
              .convertFromTimeToHour(allTimeSlotsList[i]));
        } else {
          remainingTimeSlot.add(TimeManipulativeHelperController.instance
              .convertFromTimeToHour(allTimeSlotsList[i]));
        }
      }
      availableNightTimeList.addAll(remainingTimeSlot);
    }
    setState(() {
      dataLoaded = true;
    });

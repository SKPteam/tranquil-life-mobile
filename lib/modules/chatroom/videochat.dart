...
class _VideoChatState extends State<VideoChat> {
  ...
  Future<void> initialize() async {
    ...
    await _engine.joinChannel(
        widget.agoraToken, widget.channelName, null, widget.uid.hashCode);
  }
  ...
  void _addAgoraEventHandlers() {
    _engine.setEventHandler(RtcEngineEventHandler(error: (code) {
      ...
      setState(() {
        final info = 'onJoinChannel: $channel, uid: ${widget.uid}';
        _infoStrings.add(info);
      });
    }, leaveChannel: (stats) {
      setState(() {
      ...
      setState(() {
        final info = 'userJoined: ${widget.uid}';
        _infoStrings.add(info);
        _users.add(uid);
      });
    }, userOffline: (uid, elapsed) {
      setState(() {
        final info = 'userOffline: ${widget.uid}';
        _infoStrings.add(info);
        _users.remove(uid);
      });
    }, firstRemoteVideoFrame: (uid, width, height, elapsed) {
      setState(() {
        final info = 'firstRemoteVideo: ${widget.uid} ${width}x $height';
        _infoStrings.add(info);
      });
    }));
  }
